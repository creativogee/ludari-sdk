...
describe('Manager', () => {
  ...
  describe('replica health check and cleanup', () => {
    it('should perform health check and remove inactive replicas during initialization', async () => {
      ...
      // Verify log message about cleaned replicas
      expect(mockLogger.log).toHaveBeenCalledWith(
        expect.stringMatching(/Cleaned up \d+ inactive replicas during initialization/)
      );
    ...
    it('should handle watch job without replica registration', async () => {
      const { manager } = createManager();

      await manager.initialize();

      // Simulate watch job execution
      const watchJob = await manager['storage'].findJobByName('__watch__');
      expect(watchJob).toBeDefined();

      // Call executeJob directly to test watch job behavior - should not throw
      await expect(manager['executeJob'](watchJob!)).resolves.toBeUndefined();
